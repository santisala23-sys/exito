...
export default function Login() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        alert('¡Cuenta creada! Revisa tu email para confirmar (o entra si desactivaste la confirmación).');
        ...
        if (error) throw error;
        router.push('/'); // Si entra bien, lo mandamos al Home
        router.refresh(); // Refresca para que el layout detecte la sesión
      }
    } catch (err) {
  ...
  return (
    <main style={{ 
      ...
      padding: '1rem', fontFamily: '-apple-system, system-ui, sans-serif' 
    }}>
      <form onSubmit={handleAuth} style={{ 
        backgroundColor: '#fff', padding: '2.5rem 1.5rem', 
        ...
        <div>
          ...
          <p style={{ color: '#666', fontSize: '0.9rem', marginTop: '0.5rem' }}>
            {isSignUp ? 'Crear nueva cuenta' : 'Ingresá tus datos'}
          </p>
        </div>
        ...
        <p 
          ...
          style={{ cursor: 'pointer', fontSize: '0.85rem', color: '#666', textDecoration: 'underline' }}
        >
          {isSignUp ? '¿Ya tenés cuenta? Ingresar' : '¿No tenés cuenta? Registrate'}
        </p>
      </form>
    </main>
  );
...
const buttonStyle = {
  width: '100%', padding: '1rem', backgroundColor: '#000', 
